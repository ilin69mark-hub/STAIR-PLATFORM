...
test('L-образный марш: настройка → расчёт → панель L-результата', async ({ page }) => {
  ...
  await expect(page.getByLabelText(/Ширина площадки Wp/)).toBeVisible()
  await expect(page.getByLabelText(/Ступеней нижнего марша/)).toBeVisible()
  await page.getByLabelText(/Ширина площадки Wp/).fill('1000')
  await page.getByLabelText(/Ступеней нижнего марша/).fill('6')
