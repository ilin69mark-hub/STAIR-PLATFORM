...
// TestNoTransportDependency проверяет, что application layer не зависит
// от слоя транспорта (инверсия зависимостей, DOM-0008).
func TestNoTransportDependency(t *testing.T) {
	...
	for _, file := range files {
		...
		for _, imp := range ast.Imports {
			path := strings.Trim(imp.Path.Value, `"`)
			if strings.Contains(path, "stairplatform/internal/transport") {
				t.Errorf("%s: application layer must not depend on transport: %s", file, path)
			}
		}
	}
}
