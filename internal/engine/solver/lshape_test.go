...
// Эталонный пример EDR-0005: H=2700, h0=180, S=630 → n=15, h=180,
// b=270. n1=6 → n2=9, H1=1080, H2=1620, L1=1620, L2=2430.
func TestSolveLShapeFormulas(t *testing.T) {
	res, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 1000))
	if err != nil {
...
func TestSolveLShapeComfortStepBounds(t *testing.T) {
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 1000), 599); err == nil {
		t.Fatal("comfort step below 600 must be rejected")
	}
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 1000), 641); err == nil {
		t.Fatal("comfort step above 640 must be rejected")
	}
}

func TestSolveLShapeEdgeCases(t *testing.T) {
	// n1 = 0 или n1 = n → ошибка разбивки.
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 0, mustLength(t, 1000)); err == nil {
		t.Fatal("n1 = 0 must error")
	}
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 15, mustLength(t, 1000)); err == nil {
		t.Fatal("n1 = n must error")
	}
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 16, mustLength(t, 1000)); err == nil {
		t.Fatal("n1 > n must error")
	}
	// Wp <= 0 → ошибка.
	if _, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 0)); err == nil {
		t.Fatal("Wp <= 0 must error")
	}
	// H слишком мало → n < 1.
	if _, err := SolveLShape(mustLength(t, 80), mustLength(t, 180), 1, mustLength(t, 1000)); err == nil {
		t.Fatal("rise without flight must error")
	}
	// b <= 0.
	if _, err := SolveLShape(mustLength(t, 700), mustLength(t, 350), 1, mustLength(t, 1000), 600); err == nil {
		t.Fatal("non-positive tread depth must error")
	}
}

func TestSolveLShapeDeterminism(t *testing.T) {
	a, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 1000))
	if err != nil {
		t.Fatal(err)
	}
	b, err := SolveLShape(mustLength(t, 2700), mustLength(t, 180), 6, mustLength(t, 1000))
	if err != nil {
		t.Fatal(err)
	}
	if a != b {
...
func TestSolveLShapeApplyWritesConfig(t *testing.T) {
	cfg, err := engineering.NewStairConfiguration(mustLength(t, 900), mustLength(t, 2700), engineering.FlightLShape)
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err := SolveLShape(cfg.Height, cfg.StepHeight, cfg.LowerStepCount, cfg.LandingWidth)
	if err != nil {
		t.Fatal(err)
	}
	res.Apply(cfg)
...
func TestSolveCheckedLShapeAppliesWhenValid(t *testing.T) {
	cfg, err := engineering.NewStairConfiguration(mustLength(t, 900), mustLength(t, 2700), engineering.FlightLShape)
	if err != nil {
		t.Fatal(err)
	}
	...
	cfg.LowerStepCount = 6
	cfg.LandingWidth = mustLength(t, 1000)
	_, vr, err := SolveCheckedLShape(cfg, constraint.StandardProfile("standard"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSolveCheckedLShapeLandingWidthError(t *testing.T) {
	// Wp < W → ошибка (EDR-0005 §7).
	cfg, err := engineering.NewStairConfiguration(mustLength(t, 900), mustLength(t, 2700), engineering.FlightLShape)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := SolveCheckedLShape(cfg, constraint.StandardProfile("standard")); err == nil {
		t.Fatal("Wp < W must error")
	}
}

func TestSolveCheckedLShapeBlockingRollback(t *testing.T) {
	...
	cfg.LandingWidth = mustLength(t, 1000)
	res, vr, err := SolveCheckedLShape(cfg, constraint.StandardProfile("standard"))
	if err != nil {
		t.Fatalf("solve error: %v", err)
	}
	if !vr.Blocking {
	...
	if res.StepCount == 0 {
		t.Fatal("returned LShapeResult must still hold computed values")
	}
}
