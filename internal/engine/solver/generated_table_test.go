...
func genMustLength(v float64) engineering.Length {
	l, _ := engineering.NewLength(v)
	return l
}
...
func TestGeneratedSolver4000(t *testing.T) {
	f, err := os.Open("testdata/cases-500.json")
	if err != nil {
	...
	for i, cs := range cases {
		...
		t.Run(fmt.Sprintf("%s_%d", cs.Flight, i), func(t *testing.T) {
			...
			switch cs.Flight {
			case "straight":
				_, vr, err = SolveChecked(cfg, prof, cs.ComfortStepMM)
			case "l_shape":
				_, vr, err = SolveCheckedLShape(cfg, prof, cs.ComfortStepMM)
			case "u_shape":
				_, vr, err = SolveCheckedUShape(cfg, prof, cs.ComfortStepMM)
			case "spiral":
				_, vr, err = SolveCheckedSpiral(cfg, prof)
			}
			if err != nil {
				t.Fatalf("SolveChecked err: %v", err)
			}
			if vr.Blocking {
