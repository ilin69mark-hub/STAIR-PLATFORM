...
### 3.2.1 Rate limiting за доверенным прокси (S4-2, инфра-решение)

Реализация намеренно использует только `RemoteAddr` для определения IP
клиента (`clientIP` в `middleware_auth.go`): заголовки `X-Forwarded-For` /
`X-Real-IP` игнорируются, чтобы атакующий не мог обойти лимит поддельным
заголовком (SEC). Следствие: **за прокси (nginx/LB) все запросы считаются
с одного IP** и общий лимит login/register расходуется на всех пользователей.

Решение (инфраструктурное, код не нужен, пока нет реального прокси в проде):

1. На nginx/LB пробросить реальный IP клиента доверенными заголовками
   (`X-Forwarded-For`, `X-Real-IP`), перезаписывая их на входе и беря
   значение из `$remote_addr` (не из входящего запроса).
2. В приложении задать список **доверенных прокси** (env-конфигурация,
   бэкенд-подсети балансировщика). Только для соединений из доверенного
   источника `clientIP()` читает `X-Forwarded-For` (крайний, непроксированный
   адрес); для остальных — по-прежнему `RemoteAddr`.
3. Пока список доверенных прокси пуст (продакш-прокси не развёрнут) —
   поведение остаётся текущим: `RemoteAddr`-only. Это безопасный дефолт:
   «все с одного IP» хуже для удобства, но не создаёт вектора обхода.

Инвариант: доверенный прокси фиксируется криптографически недостижимым
списком (совпадение сети + отсутствие обхода по подмене заголовка);
клиентский `X-Forwarded-For` без доверенного прокси игнорируется всегда.
...
| 1.2.0 | 2026-09-20 | §3.2.1: инфра-решение S4-2 — rate limiting за доверенным прокси (X-Forwarded-For/X-Real-IP, список доверенных прокси; код не нужен до появления реального прокси) |
