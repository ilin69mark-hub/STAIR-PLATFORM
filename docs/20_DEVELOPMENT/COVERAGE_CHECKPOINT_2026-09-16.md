...
> **Обновление 2026-09-17 (admin.go/users.go → 100%):** последние дыры админ-поверхности закрыты — `handleExport` 62.9→100%, `handleAdminOverview` 64.3→100%, `handleUpdateSettings` 72.2→100%, `handleCreateApiKey` 83.3→100%, `toApiKeyDTO` 66.7→100%, `handleListApiKeys` 75→100%, `handleRevokeApiKey` 81.8→100%, `handleListUsers`/`handleUpdateUser`/`handleGetSettings` 100%. Новые тесты в `internal/transport/http/{admin,projects}_test.go`: overview-счётчики (active/disabled/admin users, active/revoked-keys с `RevokedAt`/`LastUsedAt`) + 3 error-ветки; export audit JSON+CSV, projects CSV, invalid format, 3 service-error; api-keys DTO-мета, invalid JSON, service-error; settings/users invalid JSON + audited PUT. Расширены фейки: `adminErrAuth` (+`listUsersErr`/`listKeysErr`/`policyGetErr`, срезы `users`/`keys`), `fakeProjectService` (+`listTenantErr`), `failingWriter`. Готчи: (1) `writeExport` в JSON отдаёт массив и НЕ вызывает `toRow` — CSV-ветки (audit toRow) ловятся отдельным кейсом; (2) ветка `cw.Write(err)` = переполнение буфера `csv.Writer` — 500 длинных строк + `failingWriter`; (3) при nil projects `handleAdminOverview` падает в panic-recovery → 500 — error-тесты обязаны передавать `newFakeProjectService()`.

> **Обновление 2026-09-17 (мерж e2e/matrix-2000 в main):** ветка линейной матрицы 2000 смержена в `main` (origin/main `1792c3f`). Конфликты разрешены в пользу origin/main: multi-origin cookie (суффикс `_admin`, RFC 6265 host-only: `setSessionCookies(s,r,t)`/`sessionCookieFor`+`appOrigin`), dedup с хэшем тела (`dedupBodyHash`, FNV-1a ≤1MB → параллельные воркеры не ловят ложный 429), `X-App-Origin` в admin-клиенте, табы результата в `ResultPanel`. ВАЖНО: в `setSessionCookie` при мерже потеряно `Value: token` (cookie с пустым значением, красный `TestSsoCallbackSetsSession`) — восстановлено. Также в main наконец легли scope-валидация (`entity.go` `AllPermissions`/`IsKnown`), e2e admin-panel 7 кейсов и этот док.
